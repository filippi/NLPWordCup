...
#dkw = pd.read_pickle("lastRunAllVids.pk")
dkw = pd.read_csv("lastRunAllVids.csv")
...
categories["Voyage et evenements"] = ["barcelone","usa","evenement","aventures","aventure","vacances","qatar"]
...
allColumnNames = list(categories.keys())
allVids = pd.DataFrame(columns=allColumnNames,index=(range(len(dfs))))
...
for i, dfM in enumerate(dfs):
    ...
    for index, row in dfM.iterrows():
        for elt in allColumnNames:
            allVids.loc[i][elt] += row[elt]
...
with open('test.csv',  'w', newline='') as csvfile:
    ...
    for index, row in allVids.iterrows():
        ...
        for ni, keyRow in enumerate(rankedRow[kwStart:kwStart+kwLimit].keys()):
            ...
            if(index>0):
                lastValue=dcat.iloc[index-1][keyRow]
